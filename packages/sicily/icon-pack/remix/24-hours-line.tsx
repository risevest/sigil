import { memo } from 'react'
import type { IconProps } from '../../types'
const Ri24HoursLine = /* @__PURE__ */ memo(function Ri24HoursLine(_props: IconProps) {
  const props = _props
  return (
    <svg fill="currentColor" viewBox="0 0 24 24" width="1em" height="1em" {...props}>
      <path d="M12.005 13.003a3 3 0 0 1 2.08 5.162l-1.91 1.837h2.83v2h-6l-.001-1.724 3.694-3.555a1 1 0 1 0-1.693-.72h-2a3 3 0 0 1 3-3m6 0v4h2v-4h2v9h-2v-3h-4v-6zm-14-1a7.99 7.99 0 0 0 3 6.246v2.416a10 10 0 0 1-5-8.662zm8-10c5.185 0 9.449 3.946 9.95 9h-2.012a8.001 8.001 0 0 0-14.554-3.5h2.616v2h-6v-6h2v2.499a9.99 9.99 0 0 1 8-4" />
    </svg>
  )
})
/**
 * Remix Icon: 24 Hours Line
 * @see {@link https://remixicon.com/icon/24-hours-line Remix Icon Docs}
 */
export { Ri24HoursLine }
